...
export class TweakerService {
        ...
        try {
            ...
            const users = await Promise.all(
                profiles.map(profile =>
                    clerkClient.users.getUser(profile.userId)
                )
            );
            ...
            return profiles.map((profile, index) => ({
                ...
                username: users[index].username || profile.username || "Anonymous Tweaker"
            }));
        } catch (error) {
            console.error("Error al obtener tweakers destacados:", error);
            throw error;
        }
    }
    ...
    static async updateStats(userId: string) {
        try {
            ...
            const totalRating =
                profile.reviews.reduce((sum: number, review: Review) => sum + review.rating, 0) /
    ...
    static async toggleFeaturedTweak(userId: string, tweakId: string) {
        try {
            ...
            console.error("Error al alternar tweak destacado:", error);
            throw error;
        }
    }
} 